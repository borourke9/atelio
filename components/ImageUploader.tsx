...
interface ImageUploaderProps {
  ...
  onScaleModeChange?: (mode: 'contain' | 'cover') => void;
}
...
const ImageUploader = forwardRef<HTMLImageElement, ImageUploaderProps>(({ id, label, onFileSelect, imageUrl, isDropZone = false, onProductDrop, persistedOrbPosition, showDebugButton, onDebugClick, isTouchHovering = false, touchOrbPosition = null, scaleMode = 'contain', onScaleModeChange }, ref) => {
  ...
  const handlePlacement = useCallback((clientX: number, clientY: number, currentTarget: HTMLDivElement) => {
    ...
    if (scaleMode === 'cover') {
        if (imageAspectRatio > containerAspectRatio) {
            // Image is wider than container, so height is constrained
            renderedHeight = containerHeight;
            renderedWidth = containerHeight * imageAspectRatio;
        } else {
            // Image is taller than container, so width is constrained
            renderedWidth = containerWidth;
            renderedHeight = containerWidth / imageAspectRatio;
        }
    } else { // 'contain' is the default
        if (imageAspectRatio > containerAspectRatio) {
            // Image is wider than container, so width is constrained
            renderedWidth = containerWidth;
            renderedHeight = containerWidth / imageAspectRatio;
        } else {
            // Image is taller than container, so height is constrained
            renderedHeight = containerHeight;
            renderedWidth = containerHeight * imageAspectRatio;
        }
    }
        ...
        {imageUrl ? (
          <>
            <img 
              ...
              className={`w-full h-full ${scaleMode === 'cover' ? 'object-cover' : 'object-contain'}`} 
            />
            {onScaleModeChange && (
                <div className="absolute bottom-2 left-2 bg-black bg-opacity-60 text-white text-xs font-semibold rounded-md p-1 flex gap-1 z-20 shadow-lg">
                    <button
                        onClick={(e) => { e.stopPropagation(); onScaleModeChange('contain'); }}
                        className={`px-2 py-0.5 rounded transition-colors ${scaleMode === 'contain' ? 'bg-white text-black' : 'hover:bg-white/20'}`}
                        aria-pressed={scaleMode === 'contain'}
                    >
                        Contain
                    </button>
                    <button
                        onClick={(e) => { e.stopPropagation(); onScaleModeChange('cover'); }}
                        className={`px-2 py-0.5 rounded transition-colors ${scaleMode === 'cover' ? 'bg-white text-black' : 'hover:bg-white/20'}`}
